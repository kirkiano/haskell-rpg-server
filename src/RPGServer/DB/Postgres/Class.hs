...
instance (MonadIO m, MonadCatch m) => DriverDB (PG m) where
  createThing n pid = do
    ...
    sqlL = "insert into world_located (thing_id, place_id) values (?,?)"

  thingToCharacter tid pid = void $ pgE1 sql (tid, pid) where
    sql = "insert into world_character \
         \ (thing_ptr_id, home_id, is_logged_in) \
         \ values (?, ?, false)"
